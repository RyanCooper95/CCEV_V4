...
% state penalization
%max_delta = vstar+4+(vstar/10);%errore massimo che voglio sulla velocità
%v_max = max_delta/3.6; % [m/s] maximum allowable linear speed error
v_max = 3; % [m/s] maximum allowable linear speed error
omega_max = v_max/r; % [rad/s] maximum allowable angular speed error
Im_max = 2000; % [A] maximum allowable current error
...
grid on
hold on
plot(tcl,xcl(:,2)*9.5493,'LineWidth',1.5)
legend('Open Loop','Closed Loop')
